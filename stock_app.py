...
def add_stock():
    ...
    if ticker_symbol:
        try:
            stock_name, dividend_percent, current_price = spg.search_stock(
                ticker_symbol
            )  # 株価と配当金取得
            db.insert_data(
                ticker_symbol, stock_name, dividend_percent, current_price
            )  # 株情報新規登録
...
def update_stock_list():
    ...
    for stock in stock_data:
        ...
        dividend_ratio = (
            round(dividend_forecast_yen / dividend_sum * 100, 2)
            if dividend_sum != 0
            else 0
        )
        stock_tree.item(
...
def sort_column(tree, col, reverse):
    ...
    tree.heading(
        col, command=lambda: sort_column(tree, col, not reverse)
    )  # 再帰的に呼び出して並び替え
...
def on_cell_click(event):
    ...
    if col == "#6" or col == "#9":  # acquired_shares列をクリックした場合
        selected_item = stock_tree.item(stock_tree.selection())["values"][:8]
        if selected_item == "":
            return  # 選択されていなければ無視
        ...
        if (
            input_value is not None and input_value > acquired_shares
        ):  # 入力値は所持数より大きく
            ...
            messagebox.showerror(
                "Input Error", "Please enter a value greater than acquired shares."
            )
            return

        predicted_sum_price = (
            current_price * expected_shares
        )  # 入力した所持数に対しての合計価格
...
for i, (label_text, entry) in enumerate(fields):
    label = tk.Label(
        root, text=label_text, fg="white", bg="#222222", font=("Meiryo UI", 12, "bold")
    )
...
for col in stock_tree["columns"]:
    stock_tree.heading(
        ...
        text=col,
        anchor="e",
        command=lambda c=col: sort_column(stock_tree, c, False),
    )  # ソート機能を追加
...
delete_button = ctk.CTkButton(
    root, text="Delete Selected", command=on_delete_button_click
)
...
difference_label = tk.Label(
    root, text="差額合計: 0", bg="#222222", fg="white", font=("Meiryo UI", 12, "bold")
)
...
total_label = tk.Label(
    root, text="合計金額: 0", bg="#222222", fg="white", font=("Meiryo UI", 12, "bold")
)
total_label.grid()
...
# アプリ起動時にデータリストを更新
for stock in db.fetch_data():
    ticker_symbol = stock[1]
    stock_name, dividend_percent, current_price = spg.search_stock(
        str(ticker_symbol)
    )  # 最新情報取得
    db.current_update(ticker_symbol, dividend_percent, current_price)
    time.sleep(0.5)
update_stock_list()
